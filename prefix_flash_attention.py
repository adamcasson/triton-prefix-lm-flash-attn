...
def _fwd_kernel(Q, K, V, sm_scale,  #
    ...
    hi = N_PREFIX if start_m * BLOCK_M < N_PREFIX else (start_m + 1) * BLOCK_M
...
def _bwd_kernel_one_col_block(Q, K, V, sm_scale, qk_scale,  #
    ...
    lo = 0 if start_n * BLOCK_M < N_PREFIX else start_n * BLOCK_M
...
class _prefix_attention(torch.autograd.Function):

    @staticmethod
    def forward(ctx, q, k, v, prefix_len, sm_scale=None, sequence_parallel=False):
        q, k, v = q.contiguous(), k.contiguous(), v.contiguous()
        # only support for Ampere and greater now
        ...
        _fwd_kernel[grid](
            ...
            q.shape[0], q.shape[1], q.shape[2], prefix_len,  #
...
prefix_attention = _prefix_attention.apply
